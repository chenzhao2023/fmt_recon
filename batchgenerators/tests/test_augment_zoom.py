...
class TestAugmentZoom(unittest.TestCase):
    ...
    def test_augment_zoom_3D_dimensions(self):
        np.testing.assert_array_equal(self.zoom_factors * np.array(self.data3D.shape[1:]), np.array(self.d3D.shape[1:]), "image_x has unexpected return shape")
    ...
    def test_augment_zoom_3D_values(self):
        self.assertTrue(self.zoom_factors ** 3 * sum(self.data3D.flatten()) == sum(self.d3D.flatten()), "image_x has unexpected values inside")
        ...
        self.assertTrue(np.all(self.d3D[:, 42:82, 24:64, 26:66].flatten()), "image_x data is not zoomed correctly")
        idx = np.where(1 - self.d3D)
        tmp = self.d3D[idx]
        self.assertFalse(np.all(tmp), "image_x has unexpected values outside")
    ...
    def test_augment_zoom_2D_dimensions(self):
        np.testing.assert_array_equal(self.zoom_factors * np.array(self.data2D.shape[1:]), np.array(self.d2D.shape[1:]), "image_x has unexpected return shape")
    ...
    def test_augment_zoom_2D_values(self):
        self.assertTrue(self.zoom_factors ** 2 * sum(self.data2D.flatten()) == sum(self.d2D.flatten()), "image_x has unexpected values inside")
        ...
        self.assertTrue(np.all(self.d2D[:, 42:82, 24:64].flatten()), "image_x data is not zoomed correctly")
