...
def DeepSuperviseBlock(filters, stage, cols, classes, activation, kernel_size=(3, 3), use_batchnorm=False):
    """
    Deep supervise block: for different scale output
    """
    for i in range(stage):
        conv_name, bn_name, relu_name, up_name, merge_name = handle_block_names("stage%d-%d" % (stage, i), cols)
        filters = Conv2D(filters, kernel_size, padding="same", name=conv_name, use_bias=not (use_batchnorm))(filters)
        if use_batchnorm:
            x = BatchNormalization(name=bn_name)(x)
        x = Activation('relu', name=relu_name)(x)
    x = Conv2D(classes, (3, 3), padding='same', name='final_conv')(filters)
