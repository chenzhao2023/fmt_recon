...
class DataVisualizer(object):
    ...
    def visualize(self, num_per_row=32):
        f, plots = plt.subplots(len(self.data), num_per_row, figsize=(num_per_row, len(self.data)))
    ...
    def visualize_np(self, num_per_row=32, patch_size_x=32, patch_size_y=32, spacing=1):
        height = patch_size_x * len(self.data) + spacing * (len(self.data) - 1)
        width = patch_size_y * num_per_row + spacing * (num_per_row - 1)
        #img = np.zeros((height, width), dtype=np.uint8)
        img = np.ones((height, width), dtype=np.uint8) * 255
        for row in range(len(self.data)):
            for col in range(num_per_row):
                patch_data = self.data[row][:, :, col]
                if np.max(patch_data) - np.min(patch_data) == 0:
                    patch_data = np.array(
                        (patch_data - np.min(patch_data)) / (np.max(patch_data) - np.min(patch_data)+1e-10) * 255., dtype=np.uint8)
                else:
                    patch_data = np.array((patch_data - np.min(patch_data)) / (np.max(patch_data) - np.min(patch_data)) * 255., dtype=np.uint8)

                img[row*patch_size_x + row*spacing: (row+1)*patch_size_x + row*spacing,
                    col*patch_size_y + col*spacing: (col+1)*patch_size_y + col*spacing] = patch_data
...
class DataVisualizerGIF(object):
    ...
    def visualize(self, num_per_row=32):
        ...
        for r in range(num_per_row):
            f, plots = plt.subplots(len(self.data), 1, sharex='dicom', sharey='dicom', figsize=(1, len(self.data)))
            for row in range(len(self.data)):
                plots[row].axis('off')
